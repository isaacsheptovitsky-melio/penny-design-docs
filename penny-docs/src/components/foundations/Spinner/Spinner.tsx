...
export const Spinner = forwardRef<SpinnerProps, 'span'>(
  (
    ...
    // Only render an override when one is actually a component. The SpinnerProvider context
    // defaults to an empty object `{}` (truthy) when no provider is mounted — as in this docs
    // project — so guard against it; otherwise `createElement({})` throws "Element type is invalid".
    const hasSpinnerOverride =
      typeof spinnerOverride === 'function' ||
      (typeof spinnerOverride === 'object' && spinnerOverride !== null && '$$typeof' in spinnerOverride);
