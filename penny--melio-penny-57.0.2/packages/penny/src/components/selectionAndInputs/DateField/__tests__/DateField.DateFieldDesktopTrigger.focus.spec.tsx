...
describe('Date Field', () => {
  ...
  it('should focus calendar', async () => {
    const { user, getByRole } = renderDateField({ value: createDate('2022-09-01') });
    await user.tab();
  ...
  it('should close the calendar and focus the icon button, after the last focusable element is blurred in the calendar', async () => {
  ...
  describe('when opening the calendar through the date field', () => {
    it('should focus the icon button after returning from loading state', async () => {
    ...
    it('should focus the icon button after closing the calendar by pressing Escape', async () => {
      const { user, getByRole } = renderDateField({ value: createDate('2022-02-09') });
      await user.tab();
      await user.keyboard('[Enter]');
      expect(testKit.isCalendarOpen()).toBe(true);
    ...
    it('should focus the icon button after selecting a date', async () => {
      const { user } = renderDateField({ value: createDate('2022-02-09') });
      await user.tab();
      await user.keyboard('[Enter]');
      expect(testKit.isCalendarOpen()).toBe(true);
  ...
  describe('when opening the calendar through the icon button', () => {
    it('should focus the icon button after returning from loading state', async () => {
      const { user, rerender } = renderDateField({ value: createDate('2022-02-09') });
      await user.tab();
    ...
    it('should focus the icon button when opening the calendar and pressing Escape', async () => {
      const { user, getByRole } = renderDateField({ value: createDate('2022-02-09') });
      await user.tab();
      expect(getInput()).toHaveFocus();
