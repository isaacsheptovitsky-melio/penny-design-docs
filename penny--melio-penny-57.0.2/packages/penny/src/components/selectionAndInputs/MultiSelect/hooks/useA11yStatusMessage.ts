...
const createStatusElement = (statusElementId: string, containerId: string) => {
  ...
  statusElement.setAttribute('data-component', 'MultiSelect.A11yStatusMessage');
...
/**
 * This hook generates a unique accessible status message inside each `MultiSelect` component rendered in the DOM.
 * It's meant to indicate important state changes to screen readers.
 */
export const useA11yStatusMessage = ({ isOpen, optionsCount, emptyMessage, idPrefix }: A11yStatusMessageOptions) => {
  ...
  useEffect(() => {
    if (isOpen) {
      const result = `${optionsCount} ${optionsCount > 1 ? 'options' : 'option'} available. ${isMobile ? '' : ' Use up and down arrow keys to navigate. Press Enter key to select.'}`;
